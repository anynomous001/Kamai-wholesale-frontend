...
export function AddProductSheet({ onClose }: AddProductSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col justify-end pointer-events-none">
      <div className="w-full h-full md:max-w-[414px] mx-auto relative pointer-events-auto flex flex-col justify-end">
        {/* Scrim */}
        <div className="absolute inset-0 bg-on-background/40 backdrop-blur-sm animate-in fade-in duration-300" onClick={onClose}></div>

        {/* Overlay Container */}
        <div className="relative w-full bg-surface rounded-t-[24px] h-[85vh] flex flex-col animate-in slide-in-from-bottom duration-300 shadow-[0px_-4px_20px_rgba(45,27,20,0.08)]">
          {/* Header */}
          <div className="flex items-center justify-between px-container-margin py-md border-b border-outline-variant/30 shrink-0">
            ...
            <button className="p-2 text-on-surface-variant hover:bg-surface-container-highest rounded-full transition-colors flex items-center justify-center -mr-2" onClick={onClose}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-container-margin bg-surface border-t border-outline-variant/30 shadow-[0px_-4px_20px_rgba(45,27,20,0.04)] z-10 flex items-center justify-center shrink-0">
            <button onClick={onClose} className="w-full bg-primary text-on-primary h-[56px] rounded-xl font-label-md text-label-md flex items-center justify-center hover:bg-primary-container transition-colors shadow-sm">
              Save Product
            </button>
          </div>
        </div>
